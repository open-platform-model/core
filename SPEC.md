...
- `metadata.fqn` MUST be `"\(#moduleMetadata.registryPath):\(name):\(namespace)"`. It MUST derive from the deployed module's **major-free** `registryPath` and MUST NOT derive from the module's `fqn`, `uuid`, `version`, or major. Supplying a full module path in the registry-path position MUST be refused by `#PackagePathType` rather than yielding a third identity.
