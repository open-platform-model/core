...
#Component: {
    ...
    metadata: {
        name!:        #NameType
        resourceName: *"\(#instance.name)-\(name)" | #NameType | error("resourceName ... is not a DNS label ...")   // defaults to <instance>-<component>; override cascade
    ...
    // The default resource name, and the length assertion that applies only
    // when the default is the name in use (an explicit resourceName is
    // validated by #NameType alone).
    ...
    if metadata.resourceName == _resourceNameDefault && len(_resourceNameDefault) > 63 {
        _resourceNameDefaultFits: error("default resourceName ... is N runes, over the 63-rune DNS label limit: ...")
    }
...
- `metadata.resourceName` defaults to the instance-qualified name `"\(#instance.name)-\(metadata.name)"`. An explicit value wins via the disjunction-default cascade and MUST also satisfy `#NameType`.
- When no `resourceName` is authored and the default exceeds 63 runes, the component MUST fail validation with a diagnostic that names the concatenated string, its length, the 63-rune limit and the remedy (shorten a name or set `resourceName`). An explicit `resourceName` MUST NOT be measured against the default; `#NameType` alone validates it, and an invalid explicit value is reported as a single custom message naming the value and the DNS-label rule.
...
- **Why the default branch is not unified with `#NameType`.** Both operands are `#NameType`, so the concatenation can only fail on the 63-rune bound, and that failure must name the string. Measured on cue v0.17.1: a validated default (`*("…" & #NameType) | #NameType`) that fails degrades to a bare `incomplete value` listing `#NameType`'s constraints and never the string, and it leaves `resourceName` non-concrete, so no guard can compare against it. The unvalidated default plus the guarded `_resourceNameDefaultFits` assertion refuses the same inputs with the concatenation in the diagnostic, and leaves an explicit override to `#NameType` alone. Without the assertion the unvalidated spelling would ship a 69-rune name clean; the assertion is what makes it safe.
- **Why the diagnostics are `error()` calls, and why the assertion is a hidden field.** The built-in `error` (CUE v0.14+) replaces two measured eyesores: the overlong default's `does not satisfy strings.MaxRunes(63)` becomes a sentence naming the string, its length and what to do, and an explicit invalid name's nine nested "empty disjunction / invalid interpolation" lines (the interpolated default arm leaks a "conflicting values `shop-web` and `Bad_Name`" that reads as nonsense) become one message. The disjunction's `error` arm is reported only when every other arm fails, so the default and override paths never see it. The assertion cannot be written onto `metadata.resourceName` itself, which is the shape the CUE how-to shows: the guard reads that field, the write creates a cycle, and cue v0.17.1 resolves it by silently not applying the comprehension, so the overlong name exports clean. On a separate hidden field it fires reliably.
