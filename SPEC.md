...
#Component: {
    ...
    metadata: {
        name!:        #NameType
        resourceName: *"\(#instance.name)-\(name)" | #NameType   // defaults to <instance>-<component>; override cascade
...
- When no `resourceName` is authored and the default exceeds 63 runes, the component MUST fail validation with a diagnostic that names the concatenated string and the violated bound. An explicit `resourceName` MUST NOT be measured against the default; `#NameType` alone validates it.
