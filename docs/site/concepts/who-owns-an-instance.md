...
Kubernetes comparison, only if it helps: weave it into the sentence that introduces the concept, or into How it works, never as a section of its own. Researched candidate: Nearest idea: a Job's `spec.managedBy`, which tells the built-in Job controller to leave that Job to another controller. `spec.owner: cli` works the same way: the operator sees the ModuleInstance and deliberately does nothing but mark it. State the purpose without a tool comparison: the owner field stops the CLI and the operator from both managing one instance. Where it stops: a ModuleInstance exists in both modes and is the record for both actors; a CLI-managed one works with only the CRDs installed and no controller running, and it carries no finalizer, so it is a plain record rather than an object a controller guards. Verify: whether Job `spec.managedBy` is immutable, before contrasting it with `spec.owner`, which carries no validation rule beyond its enum. Check against: opm-operator/api/v1alpha1/moduleinstance_types.go, opm-operator/config/crd/bases/opmodel.dev_moduleinstances.yaml, opm-operator/internal/reconcile/moduleinstance.go (handleCLIOwned) -->
