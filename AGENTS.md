...
This repo defines and publishes the Open Platform Model **core schema** as a versioned CUE module (`opmodel.dev/core@v2`).
...
- For raw `cue` outside `task`, export workspace registry vars from the root `AGENTS.md` (`CUE_REGISTRY`, `OPM_REGISTRY`).
...
**Enhancement references never go in a doc comment.** Cite one as `0011:D9` — enhancement id, colon, decision id, no space; several decisions of one enhancement share a head (`0011:D16/D18/D21`), and across enhancements you repeat it (`0011:D9, 0010:D34`). A single requirement of a decision is `0011:D9:R2`; several under one decision share it (`0011:D9:R1/R2`). A bare `D9` names nothing, because decision numbers restart per enhancement. The reference belongs in the tier-2 `// WHY` block, separated from the doc comment by one blank line, because `cue lsp` hover, `Value.Doc()`, `cue def` and `task generate:index` replay a doc comment verbatim to someone who cannot open `enhancements/`. Never cite a section, slice, phase or task number (`§8.1`, `slice C2`, `task 4.2`); they are not stable identifiers. A requirement number (`R2` under a decision) is a stable identifier and is allowed. `Was:` rename history is tier-2 rationale and follows the same rule. `*_pins.cue` fixtures are exempt: their fields are hidden, so nothing replays them.
