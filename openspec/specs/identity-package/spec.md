...
Defines the shape an artifact's committed identity file must match — the two values a release moves, everything that derives from them, and the one place the schema asserts that a declared version's major agrees with the major its path declares. Every OPM artifact carries one, module and catalog alike, so the shape ships in `core` rather than in any catalog. Covers what tooling writes versus what the schema derives, why the derivation routes through a single module-path decomposition site instead of splitting the path again, why the per-kind path prefix map is enumerated rather than expressed as a pattern constraint, and why the file that must match this shape never imports the schema that states it.
