...
Defines how a published OPM artifact declares who it is. Covers the CUE module path carried by `#Module` and `#Catalog`, the major-free package path carried by the primitives a catalog ships (`#Resource`, `#Trait`, `#Blueprint`, `#ComponentTransformer`), the single construct permitted to decompose a module path (`#ArtifactRef`), and the derived `fqn` / `uuid` pair. Artifact identity distinguishes majors and nothing finer: a release does not move an artifact's UUID, a major bump does.
