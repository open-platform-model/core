...
- [ ] 1.3 Read the accumulated release-please PR. Confirm it is targeting `v1.0.0-alpha.4` and that its changelog lists the schema breaks. A misclassified commit shows up here as a missing or unexpected entry.
...
- [ ] 5.1 Merge the release-please PR. This tags `v1.0.0-alpha.4` and triggers `publish-cue`.
...
- [ ] 5.4 Verify the published artifact resolves: from a scratch tree, add `opmodel.dev/core@v1` at `v1.0.0-alpha.4` and evaluate a minimal `#Module` in the new shape. This is the first real compile of the new schema by a consumer.
...
- [ ] 6.5 Record the rollback boundary where the next slice will see it: re-pinning to `alpha.3` stays available until `library-core-retarget` merges. Put it in the `history` event, not only here.
