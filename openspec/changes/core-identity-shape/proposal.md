...
It is the root of the whole enhancement. `core-primitive-keying` and `core-platform-and-match` both depend on the types introduced here, `0011`'s `core-identity-package` cannot compile without `#ArtifactRef` and the widened `#ModulePathType`, and every downstream slice in `library`, `cli`, `opm-operator`, the catalogs and the module fleet sits behind the single cut that these four core changes feed — `v1.0.0-alpha.4` as originally scoped, **`v2.0.0-alpha.1` as of 2026-08-07** (see Impact § Version).
...
**SUPERSEDED 2026-08-07 — this IS a module-major event.** The reasoning above was sound and the conclusion was overturned by decision rather than by error: pre-stable licence made staying on `@v1` *permissible*, not *right*. The identity reshape moves every module's `uuid` and every live instance's `uuid`, redefines what `metadata.modulePath` means, and refuses values every existing artifact declares. A consumer cannot absorb that by re-reading the same import path at a higher patch — so the path moves and says so.

`opmodel.dev/core@v1` is retired at `v1.1.0-alpha.1`. This change ships on **`opmodel.dev/core@v2`** at **`v2.0.0-alpha.1`**, forced by a `Release-As:` footer, since `versioning: prerelease` advances the alpha counter within a major and never crosses one on its own.

**Rollback is no longer a re-pin.** `@v1` and `@v2` are distinct modules to CUE and to the registry; reverting means restoring the old import path as well as the old version. Both majors stay resolvable forever — a published tag names fixed bytes permanently — so the old line does not rot, but the cost of going back is an import rewrite in every consumer rather than one line in `cue.mod`.
...
**Downstream, once it is published.** The consuming set is enumerable and unchanged: `library` (compile breakage plus testdata), `cli` (address-composition helpers deleted), `opm-operator` (no feature code), `catalog_opm` / `catalog_kubernetes` / `catalog_opm_experimental`, and `modules/`. **What changed on 2026-08-07 is the cost per consumer.** Under the `@v1` plan this was a `task update-deps` sweep; on `@v2` it is an *import rewrite* — every `import "opmodel.dev/core@v1"` becomes `@v2` alongside the `cue.mod` edit, and `task update-deps` cannot do it, because resolving a new major is not the same operation as raising a version within one. Enhancement `0010`'s migration slices are correspondingly larger than scoped. Every module's `uuid` moves once, and every live instance's `uuid` moves once; the live-instance relabel is out of scope here and handled by the `v0 → v1` fleet migration (D18).
