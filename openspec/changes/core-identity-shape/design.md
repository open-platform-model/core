...
**REVISED 2026-08-07 — accepted on the enumerable-set argument alone.** The alpha argument is no longer what carries this: the module moved to `opmodel.dev/core@v2`, so the break is declared in the path rather than absorbed by pre-stable licence. What survives is the half that did the real work — every consumer is a repo in this workspace, the set is enumerable, and none of them is a third party who finds out by breaking. The retarget is now an import rewrite rather than a `task update-deps` sweep, and rollback restores an import path rather than a version pin. Both are more expensive and both are visible, which is the trade the major bump buys: a consumer that has not moved keeps resolving `@v1` at `v1.1.0-alpha.1` indefinitely instead of resolving something that no longer means what it did.
