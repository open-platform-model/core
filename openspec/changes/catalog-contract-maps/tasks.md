...
- [ ] 2.2 Add the four MUST-FAIL cases as comments (filing drift on `modulePath`, stale `catalogVersion`, a `#Resource` under `#traits`, a build-form key), each run once in place and the printed `cue vet` error recorded verbatim beneath it. Verify: with every case commented back out, `(cd src && cue vet ./...)` passes.

## 3. src/types.cue

- [ ] 3.1 Add the `#Catalog.#resources/#traits/#blueprints  map keys  #ContractFQNType` row to the "WHY unused inside `core`, per role" comment table. Comment-only. Verify: `task fmt:check` passes and the published schema is byte-identical for that file apart from the comment.

## 4. Validation

- [ ] 4.1 `task generate:index`; review that only the `#Catalog` description row changed and no new row appeared. Verify: `task generate:index:check` passes.
- [ ] 4.2 `task check` (fmt, vet, INDEX freshness, SPEC inventory, doc-comment limit) passes.
- [ ] 4.3 Grep `src/`, `SPEC.md`, `docs/` and `README.md` for "transitively" and "not enumerated" and confirm every remaining hit is about something other than catalog membership.
